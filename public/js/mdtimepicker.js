...
+(function ($) {
  ...
  var Time = function (hour, minute) {
    ...
    this.format = function (format, hourPadding) {
      ...
      return $.trim(
        format.replace(/(hh|h|mm|ss|tt|t)/g, function (e) {
    ...
    this.timepicker = {
      ...
      clockHolder: {
        ...
        buttonsHolder: {
          ...
          btnOk: $(
            `<span class="mdtp__button ok">${
              that.config.okLabel || "OK"
            }</span>`
          ),
          btnCancel: $(
            `<span class="mdtp__button cancel">${
              that.config.cancelLabel || "Cancel"
            }</span>`
          ),
        },
      },
  ...
  MDTimePicker.prototype = {
    ...
    setup: function (picker) {
      ...
      for (var i = 0; i < 12; i++) {
        ...
        hour.find("span").click(function () {
          var _data = parseInt($(this).parent().data("hour")),
            ...
            _value =
              (_data +
                ((_selectedT === "AM" && _data < 12) ||
                (_selectedT === "PM" && _data === 12)
                  ? 12
                  : 0)) %
              24;
      ...
      // Setup buttons
      clock.buttonsHolder.wrapper
        .append(clock.buttonsHolder.btnCancel)
    ...
    setHour: function (hour) {
      ...
      this.timepicker.clockHolder.clock.hours
        ...
        .each(function (idx, div) {
          ...
          el[val === that.selected.getHour(true) ? "addClass" : "removeClass"](
            "active"
          );
        });
    },
    ...
    setMinute: function (minute) {
      ...
      this.timepicker.timeHolder.minute.text(
    ...
    setT: function (value) {
      ...
      this.timepicker.clockHolder.am[t === "AM" ? "addClass" : "removeClass"](
        "active"
      );
      this.timepicker.clockHolder.pm[t === "PM" ? "addClass" : "removeClass"](
        "active"
      );
    },
    ...
    setValue: function (value) {
      ...
      var time =
        typeof value === "string"
          ? this.parseTime(value, this.config.format)
          : value;
    ...
    switchView: function (view) {
      ...
      picker.timeHolder.hour[view === "hours" ? "addClass" : "removeClass"](
        "active"
      );
      picker.timeHolder.minute[view === "hours" ? "removeClass" : "addClass"](
        "active"
      );

      picker.clockHolder.clock.hours.addClass("animate");
      if (view === "hours")
        picker.clockHolder.clock.hours.removeClass("hidden");

      clearTimeout(that.hTimeout);

      that.hTimeout = setTimeout(
        function () {
          if (view !== "hours")
            picker.clockHolder.clock.hours.addClass("hidden");
      ...
      if (view === "minutes")
        picker.clockHolder.clock.minutes.removeClass("hidden");
      ...
      that.mTimeout = setTimeout(
        function () {
          if (view !== "minutes")
            picker.clockHolder.clock.minutes.addClass("hidden");
