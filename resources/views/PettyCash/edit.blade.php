...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row">
                <div class="col-sm-12">
                    <h3 class="page-title">Petty Cash Edit</h3>
                    <ul class="breadcrumb">
                        ...
                        <div class="row my-3">
                            ...
                            <div class="col-12 col-md-6">
                                <div class="form-group ">
                                    ...
                                    <input type="date" class="form-control" name="date" id="date" value="{{\Carbon\Carbon::parse($petty_cash->date)->format('Y-m-d')}}">
                                    @error('date')
                                    <span class="text-danger">{{$message}}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6 col-12">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" name="amount" id="amount" value="{{$petty_cash->amount}}">
                                    @error('amount')
                                    <span class="text-danger">{{$message}}</span>
                                    @enderror
                                </div>
                                    ...
                                    <select name="manager_id" id="approver" class="form-control">
                                        @foreach($employees as $emp)
                                            @if($user->department_id==$emp->department_id)
                                                @if($emp->role->name=='Sales Manager'||$emp->role->name=='Finance Manager'||
                                            $emp->role->name=='HR Manager'||$emp->role->name=='Stock Manager'||
                                            $emp->role->name=='Customer Service Manager'|| $emp->role->name=='Car Admin')
                                                    <option value="{{$emp->id}}" {{$emp->id==$petty_cash->manager_id?'selected':''}}>{{$emp->name}}</option>
                                                @endif
                                            @endif
                                        @endforeach
                                    </select>
                                    ...
                                    <select name="tag_finance_id" id="approver" class="form-control">
                                        @foreach($employees as $emp)
                                            @if($emp->role->name=='Cashier')
                                                <option value="{{$emp->id}}" {{$emp->id==$petty_cash->tag_finance_id?'selected':''}}>{{$emp->name}}</option>
                                            @endif
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <textarea name="description" id="desc" cols="30" rows="5" class="form-control">{{$petty_cash->description}}</textarea>
                                </div>
                            </div>
