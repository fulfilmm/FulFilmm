                        ...
                        <div class="form-group">
                            ...
                            <select name="report_to" id="report_to" class="form-control">
                                ...
                                @foreach($all_employee as $emp)
                                    @if($emp->role->name=='Sales Manager'|| $emp->role->name=='Stock Manager'||
                        ...
                        <div class="form-group">
                            ...
                            <select name="head_office" id="head" class="form-control">
                                @foreach($head_offices as $hf)
                                    <option value="{{$hf->id}}" {{isset($employee)?($hf->id==$employee->head_office?'selected':''):old('head_office')}}>{{$hf->name}}</option>
                                @endforeach
                            </select>
                            @error('head_office')
...
<script>
    ...
    $('#branch_id').change(function () {
        ...
        @foreach($region as $item)
        if (b_id == '{{$item->branch_id}}') {
            region += '<option value="{{$item->id}}" {{isset($employee)?($employee->region_id==$item->id?'selected':''):old('region_id')}} data-option="{{$item->branch_id}}">{{$item->name}}</option>';
        }
        @endforeach
        $('#region_id').html(region);
    });
