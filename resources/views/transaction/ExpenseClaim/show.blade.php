            ...
            @if(\Illuminate\Support\Facades\Auth::guard('employee')->user()->id==$exp_claim->financial_approver && $exp_claim->status=='Approve')
                <div class="dropdown action-label">
                    <a class="btn btn-white btn-sm btn-rounded {{\Illuminate\Support\Facades\Auth::guard('employee')->user()->id==$exp_claim->approver_id?'dropdown-toggle':''}}" href="#" data-toggle="dropdown" aria-expanded="false">
                        <i class="fa fa-square{{$exp_claim->is_claim?'check':'close'}}"></i> {{$exp_claim->is_claim?'Claim':'Claimed'}}
                    </a>
                    <div class="dropdown-menu dropdown-menu-left">
                        @if($exp_claim->is_claim==0)
                            <a class="dropdown-item" href="{{route('cash.claim',$exp_claim->id)}}"><i class="fa fa-dot-circle-o text-dark"></i>Claim</a>
                            @else
                            <a class="dropdown-item text-success"><i class="fa fa-dot-circle-o text-success"></i>Has Been Claimed</a>
                        @endif
                    </div>
                </div>
                    @endif
                </div>
                @if(\Illuminate\Support\Facades\Auth::guard('employee')->user()->id==$exp_claim->approver_id && $exp_claim->is_claim==0)
                    <div class="dropdown action-label my-2">
                        <a class="btn btn-white btn-sm btn-rounded {{\Illuminate\Support\Facades\Auth::guard('employee')->user()->id==$exp_claim->approver_id?'dropdown-toggle':''}}" href="#" data-toggle="dropdown" aria-expanded="false">
                    ...
                    @elseif(\Illuminate\Support\Facades\Auth::guard('employee')->user()->id==$exp_claim->approver_id && $exp_claim->is_claim==1)
                    <div class="col-12">
                        <div class="dropdown action-label my-2">
                            <a class="btn btn-white btn-sm btn-rounded {{\Illuminate\Support\Facades\Auth::guard('employee')->user()->id==$exp_claim->approver_id?'dropdown-toggle':''}}" href="#" data-toggle="dropdown" aria-expanded="false">
                                <i class="fa fa-dot-circle-o text-{{$exp_claim->status=='New'?'secondary':($exp_claim->status=='Approve'?'success':($exp_claim->status=='Pending'?'info':'danger'))}}"></i> {{$exp_claim->status}}
                            ...
                            <div class="dropdown-menu dropdown-menu-left">
                                <a class="dropdown-item text-warning" href="#">Has Been Claimed Cash !</a>
